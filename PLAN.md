...
## Phase 3.5: 데이터 축적 대기 기간 (D+4 ~ D+30)

> **목표:** GA4 데이터 최소 4주 축적 대기 중, 인덱싱 가속 + 제품 신뢰도 강화 + Phase 4 준비
> **완료 조건:** Clarity 세션 데이터 수신, 인덱싱 요청 완료, 계산 정확도 검증 완료, E-E-A-T 강화 완료
> **블로커:** Phase 4(데이터 분석), 5-4(롱테일 확장), 5-5(수익 최적화)는 GA4/AdSense 대기로 진행 불가

### 3.5-1. P0 — 인덱싱 + 행동 데이터 수집 기반

- [ ] Search Console 인덱싱 가속 _(외부 작업)_
  - 73개 URL 중 인덱싱 현황 확인
  - 주요 페이지(메인 계산기 8개 + 홈 3개) URL 검사 → 인덱싱 요청
  - 롱테일 페이지 순차적 인덱싱 요청
- [x] Microsoft Clarity 설치 _(개발 작업)_
  - 무료 히트맵 + 세션 리플레이 도구
  - 4주간 사용자 행동 데이터 축적 → Phase 4에서 바로 분석 가능
  - 광고 위치 최적화의 데이터 기반

### 3.5-2. P1 — 제품 신뢰도 강화 (YMYL)

- [ ] 계산 엔진 정확도 검증 _(Phase 1 체크포인트 미완료)_
  - KR: 홈택스 간편계산기로 5개 소득 구간 대조
  - US: IRS Tax Withholding Estimator로 3개 시나리오 대조
  - 오차 발견 시 즉시 수정 + 테스트 추가
- [x] E-E-A-T 크레덴셜 강화 _(개발 작업)_
  - ~~About 페이지에 운영자/팀 전문성 문구 보강~~ ✅
  - ~~Schema.org `Organization` JSON-LD 추가~~ ✅
  - ~~AuthorityBlock에 "Sources verified on [date]" 추가~~ ✅

### 3.5-3. P2 — Phase 4 의사결정 품질 향상

- [ ] 경쟁사 분석 _(리서치)_
  - KR: 삼쩜삼, 홈택스 간편계산기, 찾아줘세무사 등
  - US: SmartAsset, NerdWallet tax calculator, TurboTax estimator
  - 분석 항목: 키워드 타겟, 페이지 구조, Authority 신호, 광고 배치, UX 차별점
- [ ] 다음 계산기 후보 리서치 _(기획)_
  - KR 후보: 양도소득세, 증여세, 4대보험 계산기, 연말정산 시뮬레이터
  - US 후보: Quarterly Tax, Capital Gains, Estimated Tax Penalty, State-by-State Comparison
  - 키워드 검색량 + 경쟁도 사전 조사
- [ ] UX 셀프 테스트 _(외부 작업)_
  - 8개 계산기 모바일/데스크톱 풀 시나리오 테스트
  - 입력 → 결과 → 인사이트 → 관련 계산기 이동 플로우 검증
  - 마찰점(friction) 기록 → 우선순위 매겨서 개선

### 4주 타임라인

```
Week 1 (D+4~10):  P0 — 인덱싱 요청 + Clarity 설치
Week 2 (D+11~17): P1 — 계산 정확도 검증 + E-E-A-T 강화
Week 3 (D+18~24): P2 — 경쟁사 분석 + 키워드 리서치
Week 4 (D+25~30): P2 — UX 테스트 + Phase 4 분석 준비 (GA4 대시보드 세팅)
```

### Phase 3.5 체크포인트

- [ ] Clarity 대시보드에서 세션 데이터 수신 확인
- [ ] Search Console 인덱싱 "제출됨 → 색인 생성됨" 전환 확인
- [ ] 계산 정확도 대조 결과 문서화 (비교표)
- [x] Schema.org Organization JSON-LD 추가 완료
- [ ] `npm run build` 성공
...
> **목표:** 90일간 축적된 데이터를 분석하여 데이터 기반으로 시장별 리소스 배분을 결정한다.
> **완료 조건:** KR vs US 비교 리포트 작성 + KR/US 리소스 비중 결정 + 다음 분기 로드맵 확정
...
- [ ] 비인덱싱 페이지 원인 분석 + 수정
- [ ] 고성과 키워드 식별 → Phase 5 이후 확장 우선순위 결정
...
- [ ] KR vs US 종합 평가 매트릭스 작성
  ...
  ```
- [ ] **KR/US 리소스 비중 결정** — 둘 다 운영하되 확장 우선순위, 콘텐츠 투자 비중, 롱테일 확장 방향을 데이터 기반으로 결정
- [ ] 결정 근거 문서화 (데이터 기반)

### 4-5. 다음 분기 계획 수립

- [ ] Phase 5 이후 로드맵 상세화
  - 시장별 우선순위 기준 계산기 확장 목록
  - Programmatic SEO 확장 계획 (고성과 패턴 기반)
  - 수익 목표 재설정
...
> **목표:** 계산기 포트폴리오 확장 + 데이터 기반 롱테일 확장 + 월 1만 방문자
> **완료 조건:** 계산기 10개+, 롱테일 100개+ (1차), 월 방문자 1만
...
### 5-3. 포지셔닝 재정립 — "프리랜서 전용" → "세금 계산기 허브"

> Phase 5에서 KR 3개 + US 3개 계산기를 추가하면서 직장인/사업자까지 타겟이 확장됨.
> 카피는 프리랜서 고정인데 제품은 범용 → 브랜드 일관성 괴리 해소.

- [x] 루트 랜딩 카피 + metadata 수정 — "Tax Calculator for Freelancers" → "Smart Tax Calculators"
- [x] KR 홈 재구성 — 히어로 카피 범용화 + 계산기 카테고리 분류 (프리랜서·사업자 / 직장인)
- [x] US 홈 미세 조정 — "Calculate Your 1099 Taxes" → "Tax Calculators for Self-Employed"
- [x] 메타데이터 조정 — KR/US 홈 title, description 확장
- [x] SPEC.md 타겟 페르소나 업데이트 — 프리랜서 한정 → 프리랜서, 직장인, 사업자

### 5-4. Programmatic SEO 대규모 확장

> ⚠️ 전제: Phase 4 데이터 분석 완료 후 진행

- [ ] 롱테일 자동 생성 시스템 고도화
  - 직군 × 지역 × 연도 × 소득구간 조합 자동화
- [ ] 고성과 롱테일 패턴 기반 단계적 확장 (1차 100페이지 → 성과 확인 → 2차 확장)
- [ ] Phase 4 SEO 분석 결과에 따라 US 주별 확장 범위 결정
- [ ] 페이지별 고유 인사이트 자동 생성

### 5-5. 수익 최적화

> ⚠️ 전제: Phase 3-3 AdSense 승인 완료 후 진행
...
- [ ] 월 $3,000+ 달성
- [ ] 이메일 리스트 구축 시작 (100+ 구독자)
- [ ] 월 수익 100만 원+ 달성 (AdSense + Affiliate)
